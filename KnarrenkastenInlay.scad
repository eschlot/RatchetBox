...
module continousTubes(griffTubes) 
  for(p=griffTubes) {
    startDia=p[0];
    endDia=p[1];
    length=p[2];
    ...
    translate([0,0,pos]) {
      cyl(l=length+.5,d1=startDia+.5,d2=endDia+.5,anchor=BOTTOM);// anchor_arrow(40);
      translate([(-startDia-.5)/2,0,0])
	prismoid(size1=[startDia+.5,startDia+.5],size2=[endDia+.5,endDia+.5],h=length+.5,shift=[(startDia-endDia)/2,0]);
...
module knarre(x,y,rotation,
	      ...
	      griffDaten)
{
  translate([x,y,aufbauHoehe+0.01])
    rotate(rotation)
    {
...
hexNuesse=[
		 ["Gelenk",[[0,14.1],[28,14.1],[28.1,0],[0,8.6],[35,8.6]]],
